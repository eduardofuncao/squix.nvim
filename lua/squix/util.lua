...
--- Drop leading blank/`--` comment lines so squix treats the range as inline
--- SQL rather than a saved-query name lookup.
...
--- Parse `squix list <connections|queries>` (run with NO_COLOR=1) into
--- { name, label }, skipping lines containing `skip_phrase` (e.g. "no connections").
